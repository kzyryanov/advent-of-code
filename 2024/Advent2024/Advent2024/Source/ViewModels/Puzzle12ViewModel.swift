...
final class Puzzle12ViewModel: PuzzleViewModel {
    ...
    private func regions(from plants: [[Character]]) -> Set<Region> {
        ...
        var allPlants: Set<Point> = Set(regions.keys)

        var calculatedRegions: Set<Region> = []

        while allPlants.isNotEmpty {
            let point = allPlants.removeFirst()
            ...
            let groupedVerticals = Dictionary(grouping: verticals, by: { $0.x })
            let groupedHorizontals = Dictionary(grouping: horizontals, by: { $0.y })
